...
indicator는 header 바로 아래 content와 header 사이의 중앙에 있고, text 왼쪽에 28px SVG progress ring을 둔다. `pulling`과 `armed` ring의 fill은 `clamp(rawDistance / 72, 0, 1)`이며 72px에서 완전히 찬다. `refreshing`은 full ring을 유지하고 rotation으로 전환한다. ring은 focusable하지 않고 text live region을 대체하지 않는다. 다음 text contract를 사용한다.
