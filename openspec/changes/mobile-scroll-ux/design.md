...
`prefers-reduced-motion: reduce`에서는 spring, spinner rotation과 state transition을 제거하되 손가락을 따르는 위치 피드백과 text state는 유지한다.
...
control은 상세 route에서만 render되고 header 아래 `top: 68px`, viewport horizontal center에 `z-index: 40`으로 fixed된다. visible control은 `↑ 위로`, accessible name은 `맨 위로 이동`, 정확히 `88×44px`이며 native `button`과 visible `focus-visible`을 사용한다. activation은 brand home과 같은 motion preference를 따라 document top으로 이동한다. focus를 강제로 다른 element로 옮기지 않는다.
...
|---|---|---|
| Size | 88×44px | 동일 |
| Inline padding / icon-label gap | 14px / 6px | 동일 |
| Radius | 999px | 999px |
...
label/icon은 background와 4.5:1 이상, focus indicator는 인접색과 3:1 이상을 실제 light/dark 본문 위에서 확인한다. decorative hairline은 control 식별의 유일한 단서로 사용하지 않는다. forced-colors에서 background/filter/shadow에 의존하지 않고 native button text와 focus outline이 남는지 browser QA한다.
...
- **Backdrop filter가 text contrast 또는 low-power scroll 성능을 해칠 수 있음** → 88×44 한 control에 static 12px blur만 적용하고 불투명 fallback과 실제 contrast/scroll QA를 둔다.
