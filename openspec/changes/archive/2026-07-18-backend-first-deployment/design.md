...
`workflow_dispatch`는 두 image를 모두 build/push하고 deploy한다. deploy workflow에는 `concurrency: { group: blog-production, cancel-in-progress: false }`를 둬 mutable `latest`의 동시 deploy를 직렬화한다. deploy는 필요한 build job이 모두 성공한 뒤에만 실행하며, deploy 시에는 항상 두 `latest` image를 pull하고 compose 전체를 적용한다. Oracle deploy와 backup job은 `[self-hosted, ARM64]` label을 사용한다(#20). self-hosted runner는 GHCR packages read token으로 로그인해야 하며 builder와 Oracle host 모두 `linux/arm64`를 사용한다. deploy job은 compose와 backup script를 artifact로 `/opt/blog` 및 `/opt/blog/scripts`에 배치한다. `/opt/blog/data`, `/opt/blog/backups`, `/opt/blog/scripts`를 만들고 `/opt/blog/data`와 DB를 backend runtime UID/GID `10001:10001`이 쓸 수 있게 준비한다(#21).
