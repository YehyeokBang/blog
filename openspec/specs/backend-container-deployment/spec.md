# backend-container-deployment Specification

## Purpose
Spring Boot backend image에서 SQLite 데이터를 분리해 보존하고, arm64 운영 환경의 health-gated 배포와 변경 경로별 build·deploy 순서를 안전하게 유지하는 기준을 정의한다.

## Requirements
### Requirement: Backend image safety and persistence
시스템은 backend image에 SQLite DB 또는 build cache를 포함하면 안 되며(MUST NOT), 운영 DB는 `/opt/blog/data/blog.db`에서 `/data/blog.db`로 bind mount해야 한다(MUST). image의 `/data`는 runtime UID/GID `10001:10001`이 쓸 수 있어야 한다.
...
- **AND** `/data` 디렉터리는 `10001:10001` 소유이고 container는 같은 UID/GID로 실행된다
...
시스템은 `curl`을 포함한 backend runtime image에서 healthcheck를 실행해야 하고(MUST), self-hosted runner는 health 성공 전 deploy를 성공 처리하면 안 된다(MUST NOT).
