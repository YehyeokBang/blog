...
└── posts/
    ...
    │   └── images/           ← 글에 사용된 이미지
    │       └── explain-result.png
    └── 2025-02-01-view-count-concurrency/
        ├── index.md
        └── images/
```
...
| 항목 | 상태 | 비고 |
|------|------|------|
| **블로그 이름/브랜드** | 미정 | 도메인과 연결. 우선은 IP로 운영 |
| **DB 선택 (SQLite vs PostgreSQL)** | SQLite로 시작 | Phase 2에서 동시성 문제 발생 시 마이그레이션 고려 |
| **Velog 동시 운영 전략** | 미정 | canonical URL, SEO 중복 문제에 영향 |
