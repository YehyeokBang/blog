...
backend Docker image는 build된 Boot JAR와 classpath `posts.json`만 실행한다. root `.dockerignore`는 실행 JAR만 예외로 허용하고 SQLite DB, WAL/SHM sidecar, Gradle cache, 그 밖의 backend build output을 context에서 제외한다. image는 `/data`를 만들고 UID/GID `10001:10001` 소유로 설정한 뒤 같은 runtime user로 실행한다. 운영 DB는 image가 아닌 `/opt/blog/data/blog.db`를 `/data/blog.db`로 bind mount한다.
